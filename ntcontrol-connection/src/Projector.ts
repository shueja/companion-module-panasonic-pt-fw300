...
const ENABLE_REGULAR_STATUS_POLLS = true

function isDisconnectError (err: Error | string): boolean {
    const message = typeof err === 'string' ? err : (err.message || '')
    return message === 'Socket closed.' || message === 'No socket.' || message === 'no connection available'
}
...
interface CommandStateInterface {
    ...
    setValue (value: any | undefined): any | undefined
    getName (): string
    getLabel (): string
    getValue (): any
}
...
class CommandState<T> implements CommandStateInterface {
    ...
    public setValue (value: T | undefined): T | undefined {
        if (this.value !== value) {
            this.value = value
...
export class Projector extends EventEmitter {
    ...
    // private supportsModelNameQuery: boolean = true

    // private supportsProjectorNameQuery: boolean = true
    ...
    private init (connection: Client) {
        ...
        if (ENABLE_REGULAR_STATUS_POLLS) {
    ...
    private onError (err: Error, cmd?: string) {
        if (isDisconnectError(err)) {
            this.log('debug', 'Ignoring command error while disconnected: ' + (err.message || err))
            return
        }
    ...
    // private queryDeviceMetadata () {
    //     if (this.supportsModelNameQuery) {
    ...
    //             this.queryProjectorName()
    //         }, (err) => {
    //             if (this.handleUnsupportedMetadataQuery(err, Commands.ModelNameCommand.label)) {
    //                 this.queryProjectorName()
    //                 return
    //             }
    ...
    // private queryProjectorName () {
    //     if (!this.supportsProjectorNameQuery) {
    //         return
    //     }
    ...
    //         }
    //     }, (err) => {
    //         if (this.handleUnsupportedMetadataQuery(err, Commands.ProjectorNameCommand.label)) {
    //             return
    //         }
    ...
    // private handleUnsupportedMetadataQuery (err: Error, label: string): boolean {
    //     const message = err.message || err
    //     if (message === ResponseCode.ERR1 || message === ResponseCode.ER401) {
    //         if (label === Commands.ModelNameCommand.label) {
    //             this.supportsModelNameQuery = false
    //         } else if (label === Commands.ProjectorNameCommand.label) {
    //             this.supportsProjectorNameQuery = false
    //         }

    //         this.log('debug', 'Skipping unsupported metadata query: ' + label)
    //         return true
    //     }
